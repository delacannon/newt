...
export default class MapScene extends Scene {
  ...
  generateTiles() {
    ...
    this.group = this.add.group({ key: "props", frame: arr, frameQuantity: 1 });
    ...
    this.group.getChildren().forEach((prop) => {
      prop
        ...
        .on("pointerout", () => !prop.selected && prop.setTint(0xffffff));
    });
  ...
  createIcon(tile) {
    const icon = this.add.image(0, 0, tile.icon).setScale(0.5).setInteractive();
    icon.name = tile.name;
  ...
  drawTexts() {
    EDITOR_TEXTS.forEach((config) => {
      const text = this.add
        .bitmapText(config.x, config.y, config.font, config.text, config.size)
        .setTint(config.tint)
      ...
      if (config.interactive) {
        text
          ...
          .on("pointerup", () => window.open(config.url, "_blank"));
      }
  ...
  applyUpperLayerMirrors() {
    ...
    if (this.mirror.x)
      this.placeTile(GRID_OFFSET.X - this.cursor.x, this.cursor.y, true, false);

    if (this.mirror.y)
      this.placeTile(this.cursor.x, GRID_OFFSET.Y - this.cursor.y, false, true);
    ...
    this.propsTiles = this.upperLayer.culledTiles.map(({ x, y, index }) => ({
      x,
      y,
      t: index,
    }));
  ...
  applyGridErasing() {
    ...
    if (this.mirror.x && this.mirror.y) {
      this.eraseGridTile(24 - this.cursor.x, GRID_OFFSET.Y - this.cursor.y);
    }
  }
  ...
  applyUpperLayerErasing() {
    ...
    if (this.mirror.y)
      this.eraseUpperLayerTile(this.cursor.x, GRID_OFFSET.Y - this.cursor.y);
    if (this.mirror.x && this.mirror.y) {
